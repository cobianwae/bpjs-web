...
MVC.prototype._instantiateControllers = function(){
	...
	_this._modules.forEach(function (module){
		glob.sync(path.join(module.dir, 'controllers/*.js')).forEach(function (controllerFile){
			...
			var controllerName = arrName[arrName.length-1].replace('.js','').toLowerCase().replace(/-/g,'');
			...
			for (var propertyName in controller) {
				...
				if ( _this._verbs.indexOf(verb) > -1 ) {
					if(controller instanceof APIController){
						_this._actionToAPIRoute(module, controllerName, propertyName, controller[propertyName]);
					}else{
...
MVC.prototype._setViewEngine = function () {
	var _this = this;
	console.log(_this._options.handlebarsHelpers);
	this._modules.forEach(function(module) {
		...
		module.app.engine('.html', exphbs({
			...
			defaultLayout : 'main',
			helpers : _this._options.handlebarsHelpers
		}));
...
/**
* Mapping API controller action into express route
*
* @api private
*/
MVC.prototype._actionToAPIRoute = function(module, controllerName, actionName, handler){
	var app = module.app;
	var split = actionName.split(/(?=[A-Z])/);
	var verb = split[0];
	var actionRoute = actionName.substring( verb.length, actionName.length );
	var routePath = '';
	if(actionName == 'getList' || verb == 'post' ){
		routePath = util.format('/%s', controllerName);		
	}else{
		routePath = util.format('/%s/:id', controllerName);
	}
	app[verb](routePath, handler);
};
...
MVC.prototype._beforeActionExtraction = function(module, controller, controllerName) {
	...
	var filterNames = Object.keys(beforeFilters);
	filterNames.forEach(function (actionName) {
		if (actionName === 'all') {
			app.use('/' + controllerName, beforeFilters[actionName]);
		} else {
			if (controller instanceof APIController){
				_this._actionToAPIRoute(module, controllerName, actionName, beforeFilters[actionName]);
			}else{
				_this._actionToRoute(module, controllerName, actionName, beforeFilters[actionName]);
			}
		}
	});
};
